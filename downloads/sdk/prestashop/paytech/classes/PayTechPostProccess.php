...
class PayTechPostProccess
{
    ...
    public function send()
    {
        ...
        $params = [
            'item_name' => PayTechPostProccess::arrayGet($this->query, 'item_name'),
            'item_price' => PayTechPostProccess::arrayGet($this->query, 'item_price'),
            'command_name' => PayTechPostProccess::arrayGet($this->query, 'command_name'),
            'ref_command' => $this->refCommand,
            'env' => ($this->testMode) ? 'test' : 'prod',
            'currency' => $this->currency,
            'ipn_url' => PayTechPostProccess::arrayGet($this->notificationUrl, 'ipn_url'),
            'success_url' => $this->isMobile ? $this::MOBILE_SUCCESS_URL : PayTechPostProccess::arrayGet($this->notificationUrl, 'success_url'),
            'cancel_url' => $this->isMobile ? $this::MOBILE_CANCEL_URL : PayTechPostProccess::arrayGet($this->notificationUrl, 'cancel_url'),
            'custom_field' => json_encode($this->customeField)
        ];

        $rawResponse = PayTechPostProccess::post($this::URL . $this::PAYMENT_REQUEST_PATH, $params, [
            "API_KEY: {$this->apiKey}",
